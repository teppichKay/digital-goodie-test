  ...
  validate = (state) => {
    ...
    this.validations.forEach(rule => {
      ...
      if (!validation[rule.field].isInvalid) {
        ...
        // the rule definition
        // remove spaces in the name string in order to pass the rule
        let field_value = state[rule.field].toString();
        field_value = rule.field === 'name' ? field_value.replace(/\s/g, "") : field_value;
        const args = rule.args || [];
        const locale = rule.locale || [];
        ...
        if(validation_method(field_value, ...args, ...locale, state) !== rule.validWhen) {
